...
static inline int __kaodv_queue_enqueue_entry(struct kaodv_queue_entry *entry)
{
    ...
    printk(KERN_ALERT "queue total is %d\n",queue_total);
	queue_total++;
	return 0;
}
...
static int init_or_cleanup(int init)
{
    ...
    proc = create_proc_read_entry(KAODV_QUEUE_PROC_FS_NAME, 0,
            init_net.proc_net, kaodv_queue_get_info, NULL);
#else
    proc = proc_create_data(KAODV_QUEUE_PROC_FS_NAME, 0,
            init_net.proc_net,(struct file_operations*)kaodv_queue_get_info
            ,NULL);
#endif
    if (!proc) {
...
#endif
    kaodv_queue_flush();
    printk(KERN_ERR "kaodv_queue_flush queue_total is %d\n",queue_total);
